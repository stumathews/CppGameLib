...
{	
	...
	resource_manager::resource_manager(shared_ptr<settings_manager> config,   // NOLINT(modernize-pass-by-value)
		shared_ptr<sdl_graphics_manager> graphics_admin, 
		shared_ptr<font_manager> font_admin,
		shared_ptr<audio_manager> audio_admin, std::shared_ptr<logger> the_logger)
	: config(config), graphics_admin(graphics_admin), font_admin(font_admin), audio_admin(audio_admin), the_logger(std::move(the_logger))
{
	...
	{	
		the_logger->log_message("resource_manager: reading resources.xml.");
				...
				for(auto child = pResourceTree->FirstChild(); child; child = child->NextSibling())
				{
					...
					if(element)
					{
						...
						if(the_asset)
							...
							the_logger->log_message(string("No asset manager defined for ") + type);
					}
				}
			}
		}
